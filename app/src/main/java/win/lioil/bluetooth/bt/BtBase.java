...
public class BtBase {
            ...
            while (isRead) { //死循环读取
                switch (in.readInt()) {
                    case FLAG_MSG: //读取短消息
                        ...
                        jsonMsg = msg;
                        notifyUI(Listener.MSG, "接收短消息：" + msg);  //接收到的信息在页面上显示
    ...
    public  static List<Student> stringToList(String json ){
        Gson gson = new Gson();
        List<Student> list = new ArrayList<>();
        JsonArray array = new JsonParser().parse(json).getAsJsonArray();
        for(JsonElement elem : array){
            list.add(gson.fromJson(elem, Student.class));
        }

        return list ;
    }

    public static List<Student> generateGrade(List<Student> list){
    ...
    public static String listToString(List<Student> list){
     ...
     * 发送短消息
     */
    public void sendMsg(String msg,String msg1) {
        if (checkSend()) return;
        isSending = true;
        try {
     ...
     //       mOut.writeUTF(msg1);
            ...
            notifyUI(Listener.MSG, "发送短消息：" +msg2);

//            mOut.writeUTF(msg1);
//            //       mOut.writeUTF(msg1);
//            mOut.flush();
//            notifyUI(Listener.MSG, "发送短消息：" + msg1 );
         //   notifyUI(Listener.MSG, "发送短消息1：" + msg1);
        } catch (Throwable e) {
            close();
        }
        isSending = false;
